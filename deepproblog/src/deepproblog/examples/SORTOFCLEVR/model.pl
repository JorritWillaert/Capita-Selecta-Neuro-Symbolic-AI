...
nn(cnn_red,[X],Y,[0,1,2,3,4,5,6,7]) :: detect_red_state(X,Y_red).
nn(cnn_green, [X], Y, [0,1,2,3,4,5,6,7]) :: detect_green_state(X, Y_green).

shape(red, Img, rectangle) :-
    detect_red_state(Img, Y_red),
    Y_red < 4.

shape(green, Img, rectangle) :-
    detect_green_state(Img, Y_green),
    Y_green < 4.
